...
function fitLine(samples: Sample[]): FittedLine | null {
  ...
  for (const sample of samples) {
    ...
    sy += sample.y * w;
    sx += sample.x * w;
    syy += sample.y * sample.y * w;
    syx += sample.y * sample.x * w;
    score += w;
  }
...
function pixelScore(data: Uint8ClampedArray, width: number, x: number, y: number) {
  ...
  const colourBonus = white || yellow ? 48 : 0;
  const brightnessBonus = Math.max(0, gray - 120) * 0.25;
  return edge * 1.35 + colourBonus + brightnessBonus;
}
...
function sampleSide(
  ...
  const farHalf = width * 0.07;
  const nearHalf = width * 0.38;
  ...
  for (let y = height - 5; y >= horizon; y -= 4) {
    ...
    if (bestX >= 0 && bestScore > 58) {
      samples.push({ x: bestX, y, weight: clamp(bestScore / 90, 0.5, 2.5) });
    }
  }
...
export function createLaneDetector() {
  ...
  return (video: HTMLVideoElement): LaneResult => {
    ...
    const horizonY = processHeight * HORIZON_RATIO;
    const bottomY = processHeight * 0.96;

    const scaleX = video.videoWidth / PROCESS_WIDTH;
    const scaleY = video.videoHeight / processHeight;
    const toSegment = (line: FittedLine | null): [LanePoint, LanePoint] | null => {
      if (!line) return null;
      return [
    ...
    const leftSegment = toSegment(left);
    const rightSegment = toSegment(right);
    const confidence = left && right
      ? (left.confidence + right.confidence) / 2
      : (left?.confidence ?? right?.confidence ?? 0) * 0.55;
